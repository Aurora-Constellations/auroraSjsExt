...
object BillingHtml:
    ...
    def getBillingHtml(webview: vscode.Webview, context: vscode.ExtensionContext): String = {
        val nonce = getNonce()
        // Create an HTML unordered list from the content
        // val listItems = patient.map(item => s"<li>${item.unitNumber}</li>").mkString("\n")
        val scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, "media", "ab_main.js")).toString;
        val cssUri = webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, "media", "ab_styles.css")).toString;
