...
class MeetTest extends AnyWordSpec with Matchers:
  ...
  "Meet" should {
    ...
    "intersect clinical item collections by name" in {
      ...
      (leftCoordinates |&| rightCoordinates) shouldBe
        LHSet(ClinicalItem(name = "status", narratives = LHSet(commonNote)))
      (leftValues |&| rightValues) shouldBe
        LHSet(ClinicalItem(name = "age", values = List(commonValue)))
    }
