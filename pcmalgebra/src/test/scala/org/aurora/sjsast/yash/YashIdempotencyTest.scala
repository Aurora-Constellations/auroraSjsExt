...
class YashIdempotencyTest extends AnyWordSpec with Matchers:
  ...
  "PCM merge" should {
    "be idempotent" in {
      ...
      val oc = OrderCoordinate("NAS", LHSet(NL_STATEMENT("test")), LHSet(QuReferences(LHSet(ref))))
      val ngo = NGO(name="Diet", ordercoord=LHSet(oc), narratives = LHSet(), qurefs=LHSet(QuReferences(LHSet())), qu=LHSet())
    ...
    "merge same NGO only once" in {
      ...
      val oc1 = OrderCoordinate("NAS", LHSet(), LHSet(QuReferences(LHSet(ref1))))
      val oc2 = OrderCoordinate("NAS", LHSet(), LHSet(QuReferences(LHSet(ref2))))

      val ngo1 = NGO(name="Diet", ordercoord=LHSet(oc1), narratives = LHSet(), qurefs=LHSet(QuReferences(LHSet())), qu=LHSet())
      val ngo2 = NGO(name="Diet", ordercoord=LHSet(oc2), narratives = LHSet(), qurefs=LHSet(QuReferences(LHSet())), qu=LHSet())
