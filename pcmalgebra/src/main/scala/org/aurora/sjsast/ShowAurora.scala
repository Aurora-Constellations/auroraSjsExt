...
object ShowAurora:
  ...
  // Define in dependency order: leaf types first, composite types last

  given Show[QuReference] = Show.show { (ref: QuReference) =>
    val qu = ref.qu
    val name = ref.name
    s"$qu$name"
  }

  given Show[QuReferences] = Show.show { (quRef: QuReferences) =>
