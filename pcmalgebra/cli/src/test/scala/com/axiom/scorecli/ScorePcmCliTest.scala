  ...
  "ScorePcmCli" should {
    "score a severe GCS PCM" in {
      ScorePcmCli.scoreFile(fixture("gcs-severe.aurora")).map { report =>
        ...
        val json = ujson.read(ScorePcmCli.renderJson(report))
        json("file").str shouldBe "gcs-severe.aurora"
        json("scores")("GCS_Adult")("gcs_total").num shouldBe 7.0
        json("scores")("CHA2DS2_VASc")("status").str shouldBe "not_present"
      }
    }
